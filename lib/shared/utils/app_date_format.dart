...
///    pattern string.
/// 3. **Digits are the product's, not the locale's.** `intl` renders an Arabic
///    date with Arabic-Indic numerals; every date here goes through
///    [AppNumber.latinDigits] so it matches every other number on the screen.
///    Month and day *names* stay fully localized — only the ten glyphs are
///    pinned.
///
...
class AppDateFormat {
  ...
  String day(DateTime? value) => value == null
      ? _l10n.labelUnknown
      : AppNumber.latinDigits(DateFormat.yMMMd(_localeName).format(value));
  ...
  String dayLong(DateTime? value) => value == null
      ? _l10n.labelUnknown
      : AppNumber.latinDigits(
          DateFormat('d MMMM y', _localeName).format(value),
        );
  ...
  String dateAndTime(DateTime? value) => value == null
      ? _l10n.labelUnknown
      : AppNumber.latinDigits(
          DateFormat.yMMMd(_localeName).add_Hm().format(value),
        );
